...
resource "aws_security_group" "host" {
  ...
  dynamic "ingress" {
    ...
    content {
      description = "Trusted operator SSH"
      from_port   = 22
      to_port     = 22
      protocol    = "tcp"
...
resource "aws_security_group" "data" {
  ...
  vpc_id      = aws_vpc.main.id
  ingress {
    description     = "PostgreSQL from host"
    from_port       = 5432
    to_port         = 5432
    protocol        = "tcp"
...
resource "aws_s3_bucket" "reports" {
  bucket_prefix = "${var.name}-reports-"
  force_destroy = false
}
...
resource "aws_secretsmanager_secret" "fortyguard" {
  name_prefix = "${var.name}/fortyguard/"
  description = "Populate manually; no value is managed by Terraform."
}
resource "aws_secretsmanager_secret" "backboard" {
  name_prefix = "${var.name}/backboard/"
  description = "Populate manually; no value is managed by Terraform."
}
resource "aws_secretsmanager_secret" "nvidia_nim" {
  name_prefix = "${var.name}/nvidia-nim/"
  description = "Populate manually; no value is managed by Terraform."
}
resource "aws_secretsmanager_secret" "runtime" {
  name_prefix = "${var.name}/runtime/"
  description = "Populate at deployment; no value is managed by Terraform."
}
